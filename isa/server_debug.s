...
main:
	...
	# TAG_REG    = $s2
	# BUSY_REG   = $s3
	li		$s3, 0							# li	BUSY_REG, 0
	li		$s1, 1							# li	SEND_REG, 1
	li		$s0, TEST_RECORD				# li	RECORD_REG, TEST_RECORD
	li		$s2, TEST_TAG					# li	TAG_REG, TEST_TAG
...
main_if_send_eq_1:
	...
	move	$s3, $t0						#   *busy_port = 1;
	move	$a0, $s0						#   vote_record_t rec = *rec_port;
	move	$a1, $s2						#   tag_t tag = *tag_port;

	j		process_record					#   process_record(rec, tag);
process_record_rtn:
	# ----- TESTING ONLY --------
	j		print_vote_count_table
print_vote_count_table_rtn:
	j		print_vote_count_totals
print_vote_count_totals_rtn:
	li		$v0, 11
	li		$a0, '\n'
	syscall							
	# ----- TESTING ONLY --------
